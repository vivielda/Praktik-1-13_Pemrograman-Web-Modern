...
const EffectDependenciesDemo = () => {
  ...
  // Fungsi utilitas untuk menambahkan log dengan timestamp
  ...
  useEffect(() => {
    if (name) { // Hanya log jika name terisi
      addLog(`Effect [name] dijalankan, name: ${name}`);
    }
  }, [name]);

  // // Effect 5: Multiple dependencies - run ketika count atau name
  useEffect(() => {
